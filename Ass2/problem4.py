...
a = 0.1
counts = np.zeros((len(w2i), len(w2i))) + a
...
assert np.isclose(probs[w2i['all'], w2i['the']], 0.0133657)

np.savetxt("smooth_probs.txt", np.asarray([
...
np.savetxt("smoothed_eval.txt", perplexity)
