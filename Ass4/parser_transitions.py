...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            i = 1 if transition == "LA" else 2
            self.dependencies.append((self.stack[-i], self.stack[i-3]))
            del self.stack[i-3]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        steps = model.predict(unfinished_parses[:batch_size])
        for s, pp in zip(steps, unfinished_parses[:batch_size]):
            pp.parse_step(s)
        for i in range(min(batch_size, len(unfinished_parses)) - 1, -1, -1):
            pp = unfinished_parses[i]
